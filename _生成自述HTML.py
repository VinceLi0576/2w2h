...
build_ring(
    ...
    version="v2.9",
    changelog=[
        ("v2.9", "260905", "解决即从页面消失（老徐：原文可能已被删，锚不住）；💬 列表分「未解决」与「已解决」两组，锚丢了的在列表里标出来"),
        ("v2.8", "260905", "批注可改（✎，改过标 ✎ 并记改于何时）＋ 卡片显示什么时候写的（老徐要）"),
        ("v2.7", "260905", "卡片间距压到 14、最窄 150；摆不下时把手直说「右边只剩 N px，收起目录就摆得下」，不再默默消失"),
        ("v2.6", "260905", "修：批注卡片被右侧竖条压住、文字被切 —— 可用宽度现量竖条位置再算"),
        ("v2.5", "260905", "批注卡片往外挪 40px；右下角把手可整栏收起（跟左侧目录 « 对称）；点解决同步关掉 GitHub 议题"),
        ("v2.4", "260905", "批注卡片按钮改图标不换行；已解决折成一行、鼠标移上去才出操作、点它能展开看回内容（老徐：难看）"),
