...
(function{
  ...
  /* 什么时候写的：今天只显示时分，别的显示 月-日 时:分（作者  要）*/
  function when(iso){
  ...
  /* 地方够不够摆卡，跟「目录开着没」直接相关 —— 不够时把这句话告诉他，别让卡片默默消失 */
  function roomHint{
    if(railRoom) return '';
    var ctl=document.querySelector('.ctl');
  ...
  function render{
    ...
    notes.forEach(function(a){ try{
      // 🔴 解决了就从页面上消失 —— 批注本来就是要求改文档，改完原文多半已经没了，
  ...
  function act(e){
    ...
    if(b.dataset.act==='res'){ a.resolved=!a.resolved; store.save(notes); render; pop.hidden=true;
      // 同步关／开远端议题（发过 GitHub 的才有）—— 解决了那边也该合上
