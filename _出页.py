#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""结构输出标准的**共用出页器** —— 🔑  从 <某生成器> 抽出来的。
🔴 别在各项目的生成器里再写一份骨架代码，import 这个。
📍  真身搬到 ~/.claude/skills/2w2h/；下面用法里的 <旧路径> 路径是软链接，跑顺前别改各生成器。

用法（各项目的生成器只剩几行）：
    import sys, pathlib
    sys.path.insert(0, os.path.expanduser("~/.claude/skills/2w2h"))   # 新生成器走这条；老的写 <旧路径> 的靠软链接照旧
    from _出页 import build_ring
    build_ring(src="xxx.md", dst="xxx.html", eyebrow="ops · <你的机器>",
               lead="一句话说这页解决什么", version="v1",
               changelog=[("v1","","开张")], gen_rel="<某生成器>")
...
⭐ 样式与脚本从同夹 `样式.css` `脚本.js` `批注.js` `控件.html` 读 —— **本夹就是正本**（作者「最后你改的才是标准」），不再从 Feynman 样板重抽。
"""
...
def _load_md2html:
    ...
    sys.path.insert(0, str(STD))          # 🔴 _md2html.py 就在同夹（原来在 ops 根目录，靠跨项目硬编码路径抓）
...
def body_html(md_body, first_is_lead=True, warn=True, secnum=None, subline=None):
    ...
    段头只留 序号·档位·标题，副标（subline）下移进正文第一行；小节要像 0.1／0.2 那样有号。"""
    out, sub, done_lead, k = [], None, not first_is_lead, 0
    # 副标 ＋ 结论合成一块「要点」—— 蓝方块当号位、副标当标题行、结论当正文；不带号、不折叠，
    ...
    for b in render("\n" + md_body):
        ...
        if b.startswith("§§H3§§"):
            ...
            sub = ('<details class="sub"%s><summary>%s<span class="nm">%s</span></summary>'   # 小节默认收起，点开段先看要点
                   '<div class="body">' % (sid, no, b[6:]))
            continue
        if not done_lead:
            done_lead = True
            # 🔴  某条对话线 实撞：原正则 `^<p[^>]*>` 连 `<pre data-lang>` 也吃 ⇒ 开 <p> 闭 </pre>，
    ...
    body_html.last_lead = lead_plain[0]   # 收起态段头要露的那一行
    return "".join(x for x in out if x)

# ── 页内搜索
# 样式与脚本以本夹 样式.css／脚本.js／控件.html 为正本；搜索的高亮样式与核心 JS 仍留在下面两个常量里。
...
HERO_CSS = """
/* 第 0 段 */
.tag.title{ background:var(--acc); }   /*  颜色收敛：界面只留青，标题／要点不再单独用蓝（NN/g：2 主色＋2 辅色） */
...
/* 收起态段头露一行结论；展开后隐去，正文里的要点块会显示同一句 */
...
@media (max-width:640px){ .ring{ grid-template-columns:1fr; }   /* 四钮手机单列（唯一证实的移动端缺口） */
...
(function{
  ...
  // 🔴 原脚本只认 `#sN`（getElementById 后 steps.indexOf）——
  ...
  // 🔴  实测抓到的行为不一致：原脚本的 hash 处理**只在加载时跑一次、没监听 hashchange**
  ...
  function handleHash{
    ...
    if(/^s[0-9]+-[0-9]+$/.test(h)){ var sub=document.getElementById(h); return sub && openAndScroll(sub); }   // 目录点小节写的是 #sN-k，刷新后也要认
    return gotoKey(h);
  }
  handleHash;
...
def _loc(src_p):
    """右下角那行「项目 › 文件夹」—— 🔴 只到夹，不到文件。夹在 ~/projects 下就按它算；不在就退回夹名。"""
    try:
...
def _src_commit(src_p):
    ...
    🔴  某条对话线 实撞：md 从没提交过（untracked），徽章却印了仓库 HEAD `c271adc` ——
...
def build_ring(src, dst, eyebrow, lead, version, changelog, gen_rel, title=None,
    ...
    🔴  修正：原来这行写的是「已渲染好的 HTML 片段」，**是错的，照它写会踩坑** ——
    ...
    for n, s in enumerate(secs, 1):
        ...
        if meta.get("ring"):
            ...
            # 🔴  修：ring 按钮的 w1..w4 是**按档位配色**的，不是按顺序
        ...
        parts.append(
            ...
            % (anchor, sid, (' data-key="%s"' % esc(key)) if key else '',
               '', n, tagspan, inline(s["title"]), brief, inner))   # 段头 序号·档位·标题 ＋ 收起态露一行结论（展开即隐）
    ...
    dims_html = ""
    if dims:
        # 🔴 这里原来把局部变量叫 `parts`，**撞掉了外面装 section HTML 的 `parts`**
...
<!-- 产物 · 源在 %s · 生成器 %s · 🚫 别手改本文件
     结构照 ~/projects/<旧路径>/结构输出标准.md（TL;DR ＋ 黄金圈四档） -->
<style>
%s
%s
